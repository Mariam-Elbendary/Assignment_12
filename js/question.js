...
export default class Question {
 ...
 shuffleAnswers() {
    const all = [this.correctAnswer, ...this.wrongAnswers];
  ...
  handleTimeUp() {
    ...
    document.querySelector(".answers-grid").after(msg);
    setTimeout(() => this.animateQuestion(500), 1500);
}
  ...
  checkAnswer(choiceElement) {
    ...
    this.removeEventListeners();
    this.stopTimer();
    const selected = choiceElement.getAttribute('data-answer');
  ...
  getNextQuestion() {
    ...
    if (hasMore) {
        ...
        const restart =  document.querySelector('.btn-restart')
         restart.addEventListener('click', () => this.onQuizEnd());
    }
}
